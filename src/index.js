...
async function runLoginTest(chatId) {
  ...
  try {
    await sendMessage(chatId, 'Checking existing DLD session first…');
...
async function handleUpdate(update) {
  ...
  if (command === '/uaepass') {
    await sendMessage(chatId, 'Submitting the Emirates ID stored in Railway to UAE PASS…');
