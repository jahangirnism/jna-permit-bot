...
async function ensureSession() {
  ...
  browser = await chromium.launch({ headless: false, args: ['--no-sandbox', '--disable-dev-shm-usage', '--window-size=1440,900'] });
...
function extractUaePassChallenge(text) {
  const patterns = [/(?:number|code|match|matching|shown|displayed)[^0-9]{0,40}([0-9]{2,3})/i,/([0-9]{2,3})[^0-9]{0,40}(?:number|code|match|matching)/i];
...
async function selectRealEstateAdminProfile() {
  ...
  // Select only the profile card that contains the REAL ESTATE OFFICE ADMIN type.
  ...
  if (!(await radio.count())) {
    radio = adminType.locator('xpath=following::input[@type="radio"][1]');
  }
  ...
  const proceed = await firstVisible(page, [
    'button:has-text("Proceed")','button:has-text("Continue")','button:has-text("Submit")','button:has-text("Next")','button[type="submit"]'
  ]);
  if (proceed) {
    await proceed.click();
...
async function detectState() {
  ...
  if (/multiple profiles found/i.test(bodyRaw)) {
    const selected = await selectRealEstateAdminProfile();
    if (selected?.status !== 'real_estate_admin_profile_selected') return selected;
    const newText = await page.locator('body').innerText().catch(() => '');
    if (!/multiple profiles found/i.test(newText)) return { status: 'real_estate_admin_profile_selected', url: page.url() };
    return selected;
  }
...
async function submitUaePassId() {
  ...
  const idInput = await firstVisible(page, ['input[placeholder*="Emirates ID" i]','input[aria-label*="Emirates ID" i]','input[name*="emirates" i]','input[id*="emirates" i]','input[placeholder*="ID" i]','input[type="text"]','input[type="tel"]']);
  if (!idInput) return { status: 'uae_pass_id_field_not_found', url: page.url(), browserUrl: browserUrl() };
  await idInput.fill(emiratesId);
  const loginButton = await firstVisible(page, ['button:has-text("Login")','button:has-text("Sign in")','button[type="submit"]','input[type="submit"]']);
...
async function clickTrakheesiUaePass() {
  ...
  try { if (!(await trakheesiText.isVisible({ timeout: 8000 }))) return { status: 'trakheesi_not_found', url: page.url() }; }
  catch { return { status: 'trakheesi_not_found', url: page.url() }; }
  ...
  const continueButton = await firstVisible(page, ['button:has-text("Continue with UAE PASS")','text="Continue with UAE PASS"']);
  if (continueButton) { await continueButton.click(); await page.waitForTimeout(5000); }
  return submitUaePassId();
}

export async function startInteractiveDldLogin() {
  const username = process.env.DLD_USERNAME, password = process.env.DLD_PASSWORD;
  ...
  const userInput = await firstVisible(page, ['input[name="username"]:not([type="radio"]):not([type="checkbox"])','input[name="Username"]:not([type="radio"]):not([type="checkbox"])','input[type="text"][id*="user" i]','input[type="text"][placeholder*="user" i]','input[type="email"]','input[type="text"]']);
  const passInput = await firstVisible(page, ['input[type="password"][name="password"]','input[type="password"][name="Password"]','input[type="password"][id*="pass" i]','input[type="password"][placeholder*="pass" i]','input[type="password"]']);
  if (!userInput || !passInput) return { status: 'login_form_not_found', url: page.url(), title: await page.title() };
  await userInput.fill(username); await passInput.fill(password); return detectState();
}

export async function continueAfterCaptcha() {
  if (!page || page.isClosed()) return { status: 'no_active_session' };
  const signIn = await firstVisible(page, ['button:has-text("Sign In")','button:has-text("Login")','input[type="submit"]','button[type="submit"]']);
  if (!signIn) return { status: 'signin_button_not_found', url: page.url() };
  await signIn.click(); await page.waitForTimeout(6000);
  const bodyText = (await page.locator('body').innerText().catch(() => '')).toLowerCase();
  if (bodyText.includes('trakheesi') && bodyText.includes('login with uae pass')) return clickTrakheesiUaePass();
  return detectState();
}

export async function clickTrakheesiLogin() { if (!page || page.isClosed()) return { status: 'no_active_session' }; return clickTrakheesiUaePass(); }
export async function continueUaePassLogin() { if (!page || page.isClosed()) return { status: 'no_active_session' }; return submitUaePassId(); }
export async function checkUaePassStatus() { if (!page || page.isClosed()) return { status: 'no_active_session' }; await page.waitForTimeout(1500); return detectState(); }
