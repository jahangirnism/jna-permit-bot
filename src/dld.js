...
async function pageText(p){return (await p.locator('body').innerText({timeout:1800}).catch(()=>'' )).toLowerCase();}

async function selectBestExistingPage(){
  ...
  for(const p of pages){
    ...
    else if(text.includes('emirates id, email, or phone')||text.includes('login to uae pass')||url.includes('uaepass')||url.includes('uae-pass'))score=80;
    else if(text.includes("i'm not a robot")||text.includes('recaptcha'))score=70;
    else if((await p.locator('input[type="password"]').count().catch(()=>0))>0)score=60;
    else if(url.includes('dubailand.gov.ae')||url.includes('login.dubailand.gov.ae'))score=50;
    else if(url!=='about:blank')score=10;
    scored.push({p,score});
  }
  scored.sort((a,b)=>b.score-a.score);
  if(scored[0]?.score>0){page=scored[0].p;page.setDefaultTimeout(10000);return true;}
  return false;
}
...
async function isDashboard(){const b=await pageText(page);return b.includes('dld application dashboard')||b.includes('multiple profiles found');}
...
async function handleUaePassModal(){
  ...
  await page.waitForTimeout(3000);await switchToUaePassPage();return submitUaePassId();
}
...
async function detectState(){
  const raw=await page.locator('body').innerText({timeout:5000}).catch(()=>''),text=raw.toLowerCase(),url=page.url();
  if(/multiple profiles found/i.test(raw))return selectAdmin();
  const c=challenge(raw);if(c)return{status:'uae_pass_approval_required',challenge:c,url,browserUrl:browserUrl()};
  if(text.includes('emirates id, email, or phone')||text.includes('login to uae pass')||url.toLowerCase().includes('uaepass'))return{status:'uae_pass',url,browserUrl:browserUrl()};
...
export async function startInteractiveDldLogin(){
  const u=process.env.DLD_USERNAME,p=process.env.DLD_PASSWORD;if(!u||!p)return{status:'missing_credentials'};if(!process.env.VNC_PASSWORD)return{status:'missing_vnc_password'};
  await ensureSession();

  // Reuse any meaningful page already present in the persistent browser. Do not force a fresh DLD login.
  await selectBestExistingPage();
  if(page&&page.url()!=='about:blank'){
    const existing=await detectState();
    if(['session_active','real_estate_admin_profile_selected','uae_pass','uae_pass_approval_required','captcha_required','login_form'].includes(existing.status))return existing;
  }

  // Only navigate to DLD when there is no usable existing DLD/UAE PASS page.
  await page.goto(DLD_URL,{waitUntil:'domcontentloaded',timeout:45000});await page.waitForTimeout(4000);
  if(await isDashboard()){
    const body=await pageText(page);
    if(body.includes('trakheesi')&&body.includes('login with uae pass'))return clickTrakheesi();
    await saveSession();return{status:'session_active',url:page.url()};
  }
