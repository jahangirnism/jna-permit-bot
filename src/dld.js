...
export async function continueAfterCaptcha(){
  ...
  try{
    ...
    if(body.includes('dld application dashboard')&&body.includes('trakheesi')){
      if(body.includes('login with uae pass'))return await clickTrakheesiUaePass();
    ...
    const postModal=await handleUaePassModal();
    ...
    return{status:'continue_error',message:error.message,url:page?.url?.()||''};
  }
}
