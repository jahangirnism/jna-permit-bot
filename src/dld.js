...
async function ensureSession(){if(context&&page&&!page.isClosed())return;fs.mkdirSync(PROFILE_DIR,{recursive:true});clearStaleProfileLocks();context=await chromium.launchPersistentContext(PROFILE_DIR,{headless:false,viewport:{width:1365,height:768},args:['--no-sandbox','--disable-dev-shm-usage','--window-size=1440,900']});const pages=context.pages();page=pages[0]||await context.newPage();page.setDefaultTimeout(10000);console.log(`Persistent Chromium profile active at ${PROFILE_DIR}`);}
...
async function handleUaePassModal(){
  ...
  // First try Playwright text matching inside this exact DLD modal.
  const textNode=modal.getByText(/continue\s+with\s+uae\s*pass/i,{exact:false}).last();
  if(await textNode.isVisible({timeout:1200}).catch(()=>false)){
    const clickable=textNode.locator('xpath=ancestor-or-self::*[self::button or self::a or @role="button" or contains(concat(" ",normalize-space(@class)," ")," btn ")][1]');
    if(await clickable.count()){
      try{
        await clickable.click({timeout:4000,noWaitAfter:true,force:true});
  ...
  // DOM fallback: choose the deepest/smallest element whose own visible text matches,
  // then click the nearest clickable ancestor. This avoids accidentally clicking the whole modal.
  const clicked=await page.evaluate(() => {
    const modal=document.querySelector('#application-infoUAE');
    if(!modal) return false;
    const norm=s => (s||'').replace(/\s+/g,' ').trim().toLowerCase();
    const wanted='continue with uae pass';
    const all=[...modal.querySelectorAll('button,a,[role="button"],.btn,[onclick],span,div,p')]
      .filter(el => {
        const style=getComputedStyle(el);
        const r=el.getBoundingClientRect();
        return style.visibility!=='hidden' && style.display!=='none' && r.width>0 && r.height>0 && norm(el.textContent).includes(wanted);
      })
      .sort((a,b) => norm(a.textContent).length - norm(b.textContent).length);
    if(!all.length) return false;
    const target=all[0];
    const clickable=target.matches('button,a,[role="button"],.btn,[onclick]') ? target : target.closest('button,a,[role="button"],.btn,[onclick]');
