...
async function handleUaePassModal(){
  ...
  // Preferred: any visible clickable control inside the modal explicitly mentioning UAE PASS.
  ...
  // Structural fallback confirmed from DLD UI: two action buttons, UAE PASS is the lower one.
  if(!clicked){
    clicked=await page.evaluate(()=>{
      const modal=document.querySelector('#application-infoUAE');
      if(!modal)return false;
      const els=[...modal.querySelectorAll('button,a,[role="button"],input[type="button"],input[type="submit"],.btn,[onclick]')]
...
async function detectState(){const modal=await handleUaePassModal();if(modal)return modal;const url=page.url(),raw=await page.locator('body').innerText({timeout:5000}).catch(()=>''),text=raw.toLowerCase();if(/multiple profiles found/i.test(raw))return selectAdmin();const c=challenge(raw);if(c)return{status:'uae_pass_approval_required',challenge:c,url,browserUrl:browserUrl()};const captcha=text.includes("i'm not a robot")||text.includes('recaptcha')||(await page.locator('iframe[src*="recaptcha"],iframe[title*="recaptcha" i]').count().catch(()=>0))>0;if(captcha)return{status:'captcha_required',url,browserUrl:browserUrl()};if(text.includes('authentication code'))return{status:'authentication_code',url};if(text.includes('login to uae pass')||text.includes('continue with uae pass')||text.includes('uaepass'))return{status:'uae_pass',url,browserUrl:browserUrl()};if(await isDashboard()){await saveSession();return{status:'session_active',url};}const pass=await firstVisible(page,['input[type="password"]']);if(pass)return{status:'login_form',url};return{status:'post_login_unknown',url,title:await page.title().catch(()=>'' )};}
