...
async function askNextListingField(chatId,state){
  ...
  if(!state.listingType){state.step='listing_type';return sendMessage(chatId,'Listing type? RENT or SALE');}
  ...
  if(!state.completionStatus){state.step='completion';return sendMessage(chatId,'Completion status? READY or OFF_PLAN');}
...
case'listing_type':if(!/^(rent|sale)$/i.test(v)){await sendMessage(chatId,'Please send RENT or SALE.');return true;}state.listingType=v.toUpperCase();break;
...
case'completion':if(!/^(ready|off[_ -]?plan)$/i.test(v)){await sendMessage(chatId,'Please send READY or OFF_PLAN.');return true;}state.completionStatus=/ready/i.test(v)?'READY':'OFF_PLAN';break;
...
async function createCrmAndNoc(chatId,state){
  ...
  const created=await createListingFromDraft(state);state.listingRef=created.listingRef;state.crmCreateResponse=created.data;state.step='crm_created';
  ...
  await sendDocument(chatId,pdf.buffer,pdf.fileName,`A2/NOC for CRM ${state.listingRef}. Staff ID is internal and is not shown on this PDF.`);
